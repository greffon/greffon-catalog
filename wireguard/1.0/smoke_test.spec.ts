...
test.describe('WireGuard (wg-easy)', () => {
  test('serves the wg-easy web UI login', async ({ page }) => {
    ...
    // wg-easy gates the UI behind a password login when PASSWORD is set.
    // Accept either the login surface (a password input / "Sign In") or, if a
    // build renders the app shell first, a visible body — enough to prove the
    // web UI is serving.
